...
function PhoneBook() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://phone.pinodev.shop:8000/api/phone');
      ...
      <div className='main'>
        <div className='form-container'>
          <div className='form-group'>
            ...
            />
            {errors.number && (
              <span number='text-danger'>{errors.number}</span>
            )}
            <br />
            <br />
            <button
                ...
                <tbody>
                  {values.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>{item.number}</td>
                      <td>
                        <button onClick={() => deletenum(item.idx)}>Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
